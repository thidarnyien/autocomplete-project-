    ...
    const filterProducts= products.filter((product) => {
        ...
        if(productToshow){
            for(let i=0; i<filterProducts.length; i++){
                ...
                imageTag.src=filterProducts[i].image;

                productItemContainer.append(productName,imageTag);
                resultContainer.append(productItemContainer);
            }
        }
})
let indexToSelect = -1;
const navAndSelectItem = (key) => {
    if(key==="ArrowDown"){
        indexToSelect +=1;
        const productIdToSelect = filterProducts[indexToSelect].id.toString();
        console.log("this is ", productIdToSelect)
